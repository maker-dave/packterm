#!/usr/bin/env python3
# server.py
# Version 3.0.14 - 2025-03-20  # CHANGE v3.0.14: Packet splitting, buffering, UI update, logging enhancements

# Chunk 1 v3.0.14 - Imports, Early Globals, and Utilities
import os
import sys
import time
import socket
...
def log_event(message, ui=False, submission_details=False, submissions=False, submission_payload=False, segment_failure=False, socket_state=False, retries=False, ui_transitions=False, search_query=False, search_results=False, search_parsing=False, csv_processing=False, client_state=False, packet_build=False, packet_parse=False, sync_state=False, sync_md5=False, sync_forms=False, client_packet=False, packet_integrity=False, form_deletion=False, sync_start=False, sync_completion=False, packet_queue=False, client_queue=False, ui_packet_handling=False, queue_state=False, startup_errors=False, backups=False, connection_attempts=False, packet_drop=False, thread_state=False, form_field_creation=False, form_preview=False, field_positioning=False, table_edit=False, form_save=False, ui_render=False, form_sync_error=False, packet_fragments=False, sync_mismatches=False, forms_management=False, kiss_framing=False, packet_timing=False, ax25_state=False, ax25_packet=False, kiss_packet_received=False, ax25_parse_error=False, packet_send_failure=False, socket_send_state=False, socket_send_bytes=False, socket_flush=False, socket_config=False, broadcast_state=False, socket_error=False, thread_error=False, startup_sync=False, thread_sync=False, socket_init=False, ax25_header=False, ax25_parsing_error=False, json_rebuild=False, diff_state=False, broadcast_md5=False, ax25_raw_payload=False, ax25_fcs=False, sync_broadcast=False, sync_response=False, payload_validation=False, packet_length=False, transmission_validation=False, form_content=False, packet_sanitization=False, sync_packet_validation=False, form_field_validation=False, pre_send_validation=False, packet_raw_bytes=False, form_field_sanitization=False, ax25_frame_validation=False, command_validation=False, packet_handling=False, file_io=False, filesystem_sync=False, md5_change=False, multi_packet=False, buffer_management=False):  # CHANGE v3.0.14: Added multi_packet, buffer_management
    ...
    if multi_packet and LOG_MULTI_PACKET: logging.info("Multi-Packet: " + message)  # CHANGE v3.0.14
    if buffer_management and LOG_BUFFER_MANAGEMENT: logging.info("Buffer Management: " + message)  # CHANGE v3.0.14

def log_comms(message):
    if not message.startswith(f"0{CALLSIGN}>ALL:M|"):  # CHANGE v3.0.14: Exclude MD5 broadcasts from UI
...
VERSION = "3.0.14"  # CHANGE v3.0.14: Updated version
PACLEN = 255
...
if not os.path.exists(CONFIG_FILE or ''):
    config['Settings'] = {
        ...
        'log_socket_send_bytes': 'False',  # CHANGE v3.0.14: Turned off to reduce spam
        ...
        'log_md5_change': 'True',
        'log_multi_packet': 'True',       # CHANGE v3.0.14: Added for multi-packet troubleshooting
        'log_buffer_management': 'True'   # CHANGE v3.0.14: Added for buffer troubleshooting
    }
...
LOG_SOCKET_SEND_BYTES = config.getboolean('Settings', 'log_socket_send_bytes', fallback=False)  # CHANGE v3.0.14
...
LOG_MULTI_PACKET = config.getboolean('Settings', 'log_multi_packet', fallback=True)       # CHANGE v3.0.14
LOG_BUFFER_MANAGEMENT = config.getboolean('Settings', 'log_buffer_management', fallback=True)  # CHANGE v3.0.14
...
# Chunk 2 v3.0.14 - Utility Functions
...
def build_ax25_packet(source, dest, payload):
    ...
    # CHANGE v3.0.14: Universal packet splitting
    ...
    if len(payload) > max_payload:
        ...
        return packets
    # Original single-packet logic (commented out but preserved)
...
# Chunk 3 v3.0.14 - Form Sync Functions
...
def broadcast_forms_md5(stop_event):
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            ax25_packets = build_ax25_packet(CALLSIGN, "ALL", payload)  # CHANGE v3.0.14: Now returns list
...
# Chunk 4 v3.0.14 - AX.25 Handling
        ...
        if readable:
            try:
                ...
                while b'\xC0' in buffer[1:]:
                    ...
                    # CHANGE v3.0.14: Handle multi-packet payloads
                    if ':' in payload_content[:5] and function in ['X', 'S', 'I']:  # Multi-packet responses
...
# Chunk 5 v3.0.14 - Forms Management Utils
...
# Chunk 6 v3.0.14 - People.csv Initialization
def init_people_csv():
...
# Chunk 8 v3.0.14 - Forms Management Screen
...
# Chunk 9 v3.0.14 - Form Edit Screen
...
def update_ui(stdscr):
    ...
    stdscr.addstr(1, 40, "Direwolf [Connected]", curses.color_pair(4))  # CHANGE v3.0.14: Aligned at col 40
    ...
    stdscr.addstr(max_y-1, 40, f"MD5: {forms_md5 or 'N/A'}", curses.color_pair(2))
                    ...
                    if function == 'I':
                        ...
                        if ':' in payload_content[:5]:
                            ...
                            if len(segments[key]) == total:
                                ...
                                ax25_packets = build_ax25_packet(CALLSIGN, callsign, response)  # CHANGE v3.0.14
                                for ax25_packet in ax25_packets:
                                    kiss_frame = build_kiss_packet(ax25_packet)
                                    bytes_sent = kiss_socket.send(kiss_frame)
                        ...
                        try:
                            ...
                            for fname, server_hash in server_forms.items():
                                if fname not in client_forms or client_forms[fname] != server_hash:
                                    ...
                                    ax25_packets = build_ax25_packet(CALLSIGN, "ALL", response)  # CHANGE v3.0.14
                                    for ax25_packet in ax25_packets:
                                        kiss_frame = build_kiss_packet(ax25_packet)
                                        bytes_sent = kiss_socket.send(kiss_frame)
...
# Chunk 12 v3.0.14 - Design Goals and Statuses
# DESIGN GOALS AND CHANGES:
# - See server_mini_revisions_20250320_v3.0.14.txt for this update (v3.0.14)
# - Full history in PacketRadioTerminalServiceManual [REVISION_SUMMARY]

backup_script()

# Chunk 13 v3.0.14 - Entry Point
