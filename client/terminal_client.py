#!/usr/bin/env python3
# terminal_client.py
# Version 4.3.11 - 2025-03-20  # CHANGE v4.3.11: Packet splitting, buffering, UI redesign, logging enhancements

# Chunk 1 v4.3.11 - Imports and Config
import time
import random
import sys
import os
...
VERSION = "4.3.11"  # CHANGE v4.3.11: Updated version
...
if not os.path.exists(CONFIG_FILE):
    config['Settings'] = {
        ...
        'log_ui_redraw': 'True',
        'log_socket_send_bytes': 'False',  # CHANGE v4.3.11: Turned off to reduce spam
        ...
        'log_file_io': 'True',
        'log_multi_packet': 'True',       # CHANGE v4.3.11: Added for multi-packet troubleshooting
        'log_buffer_management': 'True'   # CHANGE v4.3.11: Added for buffer troubleshooting
    }
...
LOG_MULTI_PACKET = config.getboolean('Settings', 'log_multi_packet', fallback=True)       # CHANGE v4.3.11
LOG_BUFFER_MANAGEMENT = config.getboolean('Settings', 'log_buffer_management', fallback=True)  # CHANGE v4.3.11

# Chunk 2 v4.3.11 - Global State
...
def log_event(message, ui=False, debug=False, submission=False, ui_state=False, packet=False, segment=False, submission_details=False, payload=False, segment_failure=False, socket_state=False, retries=False, ui_transitions=False, ack_processing=False, send_state=False, listener_state=False, search_query=False, search_results=False, screen_state=False, field_state=False, submission_flow=False, cursor_movement=False, packet_timing=False, error_details=False, packet_build=False, packet_parse=False, sync_state=False, sync_md5=False, sync_forms=False, sync_packets=False, packet_integrity=False, listener_retries=False, socket_reset=False, connection_success=False, packet_fragments=False, sync_mismatches=False, redraw_triggers=False, form_deletion=False, sync_start=False, sync_completion=False, form_exit=False, key_context=False, mode_switch=False, packet_queue=False, listener_queue=False, ui_packet_handling=False, queue_state=False, connection_attempts=False, packet_drop=False, thread_state=False, column_navigation=False, form_layout=False, row_movement=False, form_display_error=False, ui_render=False, socket_errors=False, form_ui_layout=False, input_field_state=False, kiss_framing=False, ax25_state=False, ax25_packet=False, ax25_parse_error=False, kiss_packet_received=False, packet_validation=False, md5_comparison=False, packet_relay=False, ui_redraw=False, socket_send_bytes=False, socket_send_failure=False, socket_reconnect=False, socket_status=False, socket_send_raw=False, socket_buffer=False, ui_comms_log=False, packet_send_time=False, packet_enqueue_time=False, packet_dequeue_time=False, queue_size=False, redraw_timing=False, kiss_receive_buffer=False, kiss_frame_timing=False, packet_content=False, socket_send_attempt=False, ui_packet_display=False, packet_structure=False, socket_validation=False, packet_transmission=False, ax25_build=False, ax25_validation=False, kiss_validation=False, fcs_calculation=False, json_rebuild=False, diff_state=False, delimiter_usage=False, sync_index=False, packet_format=False, packet_raw_decode=False, form_file_write=False, form_field_parse=False, newline_handling=False, file_content=False, command_validation=False, packet_handling=False, file_io=False, multi_packet=False, buffer_management=False):  # CHANGE v4.3.11: Added multi_packet, buffer_management
    ...
    if multi_packet and LOG_MULTI_PACKET: debug_log.append((message, timestamp))       # CHANGE v4.3.11
    if buffer_management and LOG_BUFFER_MANAGEMENT: debug_log.append((message, timestamp))  # CHANGE v4.3.11

def log_comms(message):
    if not message.endswith(":M|SVR001|NONE|"):  # CHANGE v4.3.11: Exclude M packets from UI
...
def build_ax25_packet(source, dest, payload, source_ssid=0, dest_ssid=0):
    """Build a proper AX.25 UI frame with CRC-16 FCS, splitting if needed."""
    ...
    if len(payload) > max_payload:
        log_event(f"Payload exceeds max ({max_payload}): {len(payload)} bytes, splitting", error_details=True, multi_packet=True)  # Fixed: use error_details
        ...
        for i, part in enumerate(parts):
            ...
            payload_bytes = tagged_payload.encode('ascii')
    ...
    return [ax25_packet]  # Return as list for consistency
...
# Chunk 4 v4.3.11 - Core Display Functions
...
def redraw_screen(stdscr, sending=False):
    ...
    if form_fields and any(form_fields) and mode in ('I', 'S'):
        ...
        # CHANGE v4.3.11: Redesigned main screen layout
        ...
        for i, local_form_id in enumerate(form_list[:15], 1):
            ...
            desc = desc[:30]  # Truncate to fit ~35 chars
...
# Chunk 5 v4.3.11 - Screen Transition Functions
...
# Chunk 6 v4.3.11 - AX.25 Communication
def send_to_kiss(stdscr, override_packet):
    ...
    try:
        ax25_packets = build_ax25_packet(CALLSIGN, "SVR001", packet, source_ssid=CALLSIGN_SSID, dest_ssid=0)  # CHANGE v4.3.11: Returns list
        if LOG_AX25_PACKET:
            log_event(f"AX.25 packets built: {len(ax25_packets)} parts", ax25_packet=True)
    except Exception as e:
        log_event(f"Failed to build AX.25 packets: {str(e)}", ax25_build=True, error_details=True)
        sending = False
        return
...
def kiss_listener(stdscr, stop_event):
    global comms_log, screen_dirty, messages, unread_messages, kiss_socket, last_no_data, sending, submission_result, syncing, packet_queue, socket_connected, form_parts
                        ...
                        if data:
                            ...
                            while b'\xC0' in buffer[1:]:
                                ...
                                try:
                                    ...
                                    # CHANGE v4.3.11: Handle multi-packet payloads
                                    if ':' in payload_content[:5] and function in ['U', 'R']:
                                        seq_total, content = payload_content.split('|', 1)
                                        seq, total = map(int, seq_total.split(':'))
                                        key = f"{form_id}"
                                        form_parts.setdefault(key, {})[seq] = content
                                        log_event(f"Received part {seq}/{total} for {key}", multi_packet=True, buffer_management=True)
                                        if len(form_parts[key]) == total:
                                            full_payload = ''.join(form_parts[key][i] for i in range(1, total + 1))
                                    ...
                                    try:
                                        ...
                                        if function == 'U':
                                            if LOG_COMMAND_VALIDATION:
                                                log_event(f"Validated command 'U' as FORM_UPDATE", command_validation=True)
                                            file_path = os.path.join(FORMS_DIR, f"{form_id}.txt")
                                            ...
                                            if LOG_COMMAND_VALIDATION:
                                                log_event(f"Validated command 'D' as FORM_DELETE", command_validation=True)
                                            file_path = os.path.join(FORMS_DIR, f"{form_id}.txt")
...
# Chunk 7 v4.3.11 - Main Loop (Navigation & Submit)
def main(stdscr):
    ...
    log_event(f"Script v4.3.11 started", debug=True)  # CHANGE v4.3.11: Updated version
            ...
            try:
                ...
                if function == 'M' and not syncing:
                    ...
                    if server_hash != client_hash:
                        ...
                        if LOG_SYNC_INDEX:
                            log_event(f"X (INDEX) packet built: {update_packet}", sync_index=True)
                        if LOG_SYNC_PACKETS:
                            log_event(f"Sending X (INDEX) packet: {update_packet}", sync_packets=True)
                    ...
                    screen_dirty = True
                elif function not in ['M', 'A', 'R', 'U', 'D', 'G', 'C']:
...
# Chunk 9 v4.3.11 - Results Screen
...
# Chunk 10 v4.3.11 - Main Loop (Exit Only)
...
# Chunk 11 v4.3.11 - Design Goals and Statuses
# DESIGN GOALS AND CHANGES:
# - See client_mini_revisions_20250320_v4.3.11.txt for this update (v4.3.11)
# - Full history in PacketRadioTerminalServiceManual [REVISION_SUMMARY]                except (ConnectionResetError, OSError) as e:
